...
qboolean G2API_SetBoneAnglesOffsetIndex(
	...
	qhandle_t *modelList, int blendTime, int currentTime, const vec3_t offset)
{
	qboolean setPtrs = qfalse;
	bool res = false;
	...
	if (res)
	{
		// ensure we flush the cache
		ghlInfo->mSkelFrameNum = 0;
		return G2_Set_Bone_Angles_Index(ghlInfo, ghlInfo->mBlist, index, angles, flags, yaw, pitch, roll, blendTime, currentTime, offset);
	}
...
qboolean G2API_SetBoneAnglesIndex(CGhoul2Info *ghlInfo, const int index, const vec3_t angles, const int flags,
							 const Eorientations yaw, const Eorientations pitch, const Eorientations roll,
							 qhandle_t *, int blendTime, int currentTime )
{
	return G2API_SetBoneAnglesOffsetIndex(ghlInfo, index, angles, flags, yaw, pitch, roll, 0, blendTime, currentTime, NULL);
}

qboolean G2API_SetBoneAnglesOffset(CGhoul2Info* ghlInfo, const char* boneName, const vec3_t angles, const int flags,
	const Eorientations up, const Eorientations left, const Eorientations forward, qhandle_t* modelList, int blendTime, int currentTime, const vec3_t offset)
{
	//rww - RAGDOLL_BEGIN
	if (ghlInfo && ghlInfo->mFlags & GHOUL2_RAG_STARTED)
	{
...
int G2API_AddBolt(CGhoul2Info* ghlInfo, const char* boneName, qboolean requireDrawn)
{
	...
	if (boneName && G2_SetupModelPointers(ghlInfo))
	{
		return G2_Add_Bolt(ghlInfo, ghlInfo->mBltlist, ghlInfo->mSlist, boneName, requireDrawn);
	}
	return -1;
}
